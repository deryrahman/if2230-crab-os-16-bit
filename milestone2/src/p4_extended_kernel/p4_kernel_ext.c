...
main(){
    ...
    while(1){
    	...
    	if(strcmp(buffer,"type ",5)){
    		...
    		interrupt(0x21, DELETE_FILE, buffer+7, wdir, 0);
    	...
    	} else if (strcmp(buffer,"createdir ",10)){
    		interrupt(0x21, CREATE_DIR, buffer+10, wdir, 0);
...
void writeFile(char* filename,char* wdir) {
	...
	for (i = 0; i < 6; i++)
		dir[fpos*32+i] = filename[i];
	for (i = 6; i < 12; i++)
		dir[fpos*32+i] = wdir[i-6];
...
void deleteFile(char* name, char* wdir){
	char map[512],dir[512],file[32];
	int i,j,found;
	
	readSector(map, 1);
	readSector(dir, 2);
	
	found = getFileLoc(file, name);
	if (isDir(file))
		deleteDirectory(name);
	else if (strcmp(wdir, file+6, 6)) {
		for (i = 12; i < 32; i++)
			map[dir[found*32+i]] = 0;
		for (i = 0; i < 32; i++)
			dir[found*32+i] = 0;
		writeSector(map, 1);
		writeSector(dir, 2);
...
void deleteDirectory(char* dirname) {
	int idx,i,j;
	char dir[512],map[512],buff[32];
	
	readSector(map, 1);
	readSector(dir, 2);
	idx = getFileLoc(buff, dirname);
	if (idx > -1 && isDir(buff)) {
		for (j = 12; j < 32; j++)
			map[dir[idx*32+j]] = 0;
		for (j = 0; j < 32; j++)
			dir[idx*32+j] = 0;
		for (i = 0; i < 16; i++)
			if (strcmp(dir + (32*i) + 6, dirname, 6)) {
				if (isDir(dir + (32*i)))
					deleteDirectory(dir+32*i);
				for (j = 12; j < 32; j++)
					map[dir[i*32+j]] = 0;
				for (j = 0; j < 32; j++)
					dir[i*32+j] = 0;
